"""Script de pesquisa quantitativa para ETFs e ações do mercado chinês."""
...
OUTPUT_DIR = ROOT / "results" / "china_research"
...
def fetch_or_load(ticker: str, start_date: str = "2020-01-01") -> pd.DataFrame | None:
    ...
    try:
        ...
        print(f"[{ticker}] Erro no download: {e}")
        return None
...
def run_analysis():
    ...
    df_momentum = pd.DataFrame(records).sort_values(by="sharpe_ratio", ascending=False).reset_index(drop=True)
    df_bh = pd.DataFrame(bh_records).sort_values(by="bh_sharpe", ascending=False).reset_index(drop=True)

    df_momentum.to_csv(OUTPUT_DIR / "china_momentum_metrics.csv", index=False)
    df_bh.to_csv(OUTPUT_DIR / "china_bh_metrics.csv", index=False)

    merged = pd.merge(df_momentum, df_bh, on=["ticker", "categoria"])
    merged.to_csv(OUTPUT_DIR / "china_combined_metrics.csv", index=False)

    print("\n" + "=" * 80)
    print("MOMENTUM ATR (TREND FOLLOWING COM TRAILING STOP):")
    print(df_momentum.to_string(index=False))

    print("\n" + "=" * 80)
    print("BUY & HOLD (COMPRAR E MANTER):")
